    ...
    return (
        <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md border-b border-gray-200 dark:border-white/5 py-3' : 'bg-transparent py-5'}`}>
            <div className="container mx-auto px-6 flex justify-between items-center">
                <Link to="/" className="text-2xl font-bold bg-gradient-to-r from-green-400 to-blue-400 bg-clip-text text-transparent" onClick={() => scrollToSection('home')}>
                ...
                <div className="hidden md:flex gap-8 text-sm font-medium text-gray-500 dark:text-gray-400 items-center">
                    {isHomePage ? navItems.map((item) => (
                        <button
                            ...
                            className="hover:text-green-500 dark:hover:text-green-400 transition-colors uppercase tracking-wider text-xs"
                        >
                            {item}
                        </button>
                    )) : (
                        <Link to="/" className="hover:text-green-500 dark:hover:text-green-400 transition-colors uppercase tracking-wider text-xs">Home</Link>
                    )}
                    <Link 
                        to="/blog" 
            ...
            {isMenuOpen && (
                <div className="md:hidden absolute top-full left-0 w-full bg-white dark:bg-[#0a0a0a] border-b border-gray-200 dark:border-white/5 py-4 px-6 flex flex-col gap-4">
                    {isHomePage ? navItems.map((item) => (
                        ...
                        <Link to="/" className="py-2 hover:text-green-500 dark:hover:text-green-400 transition-colors uppercase tracking-wider text-xs font-medium" onClick={() => setIsMenuOpen(false)}>Home</Link>
                    )}
