import React from 'react';
import { BarChart3, Layout, Database, Smartphone, Terminal } from 'lucide-react';

const TechStack = () => {
    const skills = [
        {
            category: "Data & AI",
            icon: <BarChart3 className="w-5 h-5" />,
            items: ["Python", "Jupyter", "Data Analysis", "ComfyUI"]
        },
        {
            category: "Frontend",
            icon: <Layout className="w-5 h-5" />,
            items: ["React", "Next.js", "Vue.js", "TypeScript", "Tailwind", "SPA Architecture"]
        },
        {
            category: "Backend",
            icon: <Database className="w-5 h-5" />,
            items: ["Node.js", "Express", "Bun", "Rust", "SQLite", "MongoDB"]
        },
        {
            category: "Platform & Desktop",
            icon: <Smartphone className="w-5 h-5" />,
            items: ["Flutter", "React Native", "Tauri"]
        },
        {
            category: "DevOps & Tools",
            icon: <Terminal className="w-5 h-5" />,
            items: ["Docker", "Git", "n8n", "Linux", "Vercel"]
        }
    ];

    return (
        <section id="stack" className="py-24 relative overflow-hidden transition-colors duration-300">
            <div className="container mx-auto px-6 relative z-10">
                <div className="text-center mb-16">
                    <h2 className="text-4xl font-bold mb-4 text-gray-900 dark:text-white">Technologies & Tools</h2>
                    <p className="text-gray-600 dark:text-gray-400">Core technical proficiencies across the stack</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {skills.map((group, index) => (
                        <div key={index} className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/5 rounded-2xl p-8 hover:border-blue-500/30 transition-all group shadow-sm dark:shadow-none">
                            <div className="flex items-center gap-4 mb-6">
                                <div className="p-3 bg-blue-50 dark:bg-white/10 rounded-xl text-blue-600 dark:text-blue-300 group-hover:scale-110 transition-transform">
                                    {group.icon}
                                </div>
                                <h3 className="text-xl font-bold text-gray-900 dark:text-white">{group.category}</h3>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {group.items.map((skill) => (
                                    <span key={skill} className="px-3 py-1 bg-gray-100 dark:bg-black/40 rounded-full text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-white/5">
                                        {skill}
                                    </span>
                                ))}
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </section>
    );
};

export default TechStack;
